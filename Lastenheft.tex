...
The user can choose an available platform:
...
\subsubsection {Choose-Operation-Section}
 ...
 The user can choose a running mode:
...
The result section contains results of the selected image or the aggregation. The results are represented in form of text. The results are the probabilities of the top detected objects in percentages.
...
The image section is used to add images. Thumbnails are displayed for the chosen image. An image can be removed by clicking on the remove-button. All images can be removed at once if the user clicks on the reset-button.
...
By using the control section the user is able to start and cancel the process. Once the classification starts, the start button becomes a cancel button. While running the user can pause the classification by clicking on the pause button. In this case the pause button becomes a resume button. The aggregate button starts the aggregation of results.

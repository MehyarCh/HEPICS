...
Convolutional_layer::Convolutional_layer(unique_ptr<Image> filters, vector<float> bias, size_t stride, size_t pad, size_t groups) :
...
unique_ptr<Image> Convolutional_layer::forward_layer(const Image &input) {
	...
	auto out_channels = filters->num;

	auto out = make_unique<Image>(out_width, out_height, out_channels, 1);

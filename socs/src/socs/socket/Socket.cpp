...
static auto connect_client_socket(const Fd &fd, const sockaddr_in &addr) {
...
static auto ensure_valid_range(const vector<uint8_t> &data, size_t offset,
		size_t length) {
...
static auto bind_server_socket(const Fd &fd, const sockaddr_in &addr) {
...
static auto start_listening(const Fd &fd) {
	if (listen(fd.fd, 1) != 0) {
		throw Listen_failed { };
	}
}

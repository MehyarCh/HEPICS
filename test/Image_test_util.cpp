...
unique_ptr<Image> Image_test_util::load_dump(const string &path) {
	...
	auto dump = make_unique<Image>(width, height, channels, num);
...
bool Image_test_util::near_equals(const Image &a, const Image &b, float max_error) {
...
bool Image_test_util::min_max_intersect(const Image &a, const Image &b, size_t radius, float max_error) {
	if (a.num != b.num || a.channels != b.channels ||
			a.height != b.height || a.width != b.width ||
...
const char *Load_dump_failed::what() const noexcept {
	return "hepics::Load_dump_failed";
}

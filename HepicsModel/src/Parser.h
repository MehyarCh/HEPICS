...
public:
	...
	static Network *parse_network_cfg(char *filename) {
		...
		Network *net = new Network(sections->getSize() - 1);
		size_params params;
		...
		while (n) {
			...
			net->getLayers()[count] = *l;
			net->getLayers()[count].setType(l->getType());
	...
	static void load_weights_upto(Network *net, char *filename, int start,
		...
		for (int i = start; i < net->getN() && i < cutoff; ++i) {
			Layer l = net->getLayers()[i];
			if (l.getType() == Layer_Type::CONVOLUTIONAL) {
				load_convolutional_weights(l, fp);
			}
			if (l.getType() == Layer_Type::CONNECTED) {
